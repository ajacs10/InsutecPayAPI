// app/telas/financeiro/CarteiraScreen.tsx
...
const AtlanticoCard = ({ isDarkMode }: { isDarkMode: boolean }) => {
  ...
  const getUserName = () => {
    if (aluno?.nome) {
      ...
      if (names.length >= 2) return `${names[0]} ${names[names.length - 1]}`.toUpperCase();
  ...
  return (
    <TouchableOpacity activeOpacity={0.9} onPress={handleFlip}>
      ...
      <Animated.View style={[
        ...
        <Text style={carteiraStyles.cardBackText}>Cartão Universitário - Para uso no sistema Insutec Pay</Text>
      </Animated.View>
    </TouchableOpacity>
  );
};

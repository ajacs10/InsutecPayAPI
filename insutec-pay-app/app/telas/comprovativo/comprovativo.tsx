...
const getLuandaDateTime = () => {
  const now = new Date();
  const luandaOffset = 1 * 60;
  const utc = now.getTime() + now.getTimezoneOffset() * 60000;
  const luandaTime = new Date(utc + luandaOffset * 60000);
  const data = luandaTime.toLocaleDateString('pt-AO', { day: '2-digit', month: '2-digit', year: 'numeric' });
  const hora = luandaTime.toLocaleTimeString('pt-AO', { hour: '2-digit', minute: '2-digit' });
  return { data, hora };
};
...
export default function ComprovativoScreen() {
  ...
  const handleGerarRecibo = async (formato: 'docx' | 'pdf' | 'txt') => {
    ...
    try {
      await gerarRecibo(selectedComprovativo.id, formato);
    } catch (error) {
            ...
            >
              ...
              <View style={sharedStyles.card(isDarkMode)}>
                <View style={{ alignItems: 'center', marginVertical: 16 }}>
                  <QRCode value={selectedComprovativo.qrCode || selectedComprovativo.id} size={160} />
                </View>
                ...
                <View style={styles.row}>
                  ...
                  <Text style={sharedStyles.value(isDarkMode)}>
                    {aluno?.curso} ({aluno?.ano_academico}ºAno)
                  </Text>
                </View>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginVertical: 20, flexWrap: 'wrap' }}>
                <TouchableOpacity style={styles.actionButton} onPress={() => handleGerarRecibo('pdf')} disabled={downloadingId !== null}>
                  <Feather name="file-text" size={20} color="#fff" />
                  <Text style={styles.actionText}>PDF</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.actionButton} onPress={() => handleGerarRecibo('txt')} disabled={downloadingId !== null}>
                  <Feather name="download" size={20} color="#fff" />
                  <Text style={styles.actionText}>TXT</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.actionButton} onPress={() => handleGerarRecibo('docx')} disabled={downloadingId !== null}>
                  <Feather name="file-word" size={20} color="#fff" />
                  <Text style={styles.actionText}>DOCX</Text>
                </TouchableOpacity>
              </View>
              <TouchableOpacity style={{ alignSelf: 'center', marginBottom: 20 }} onPress={() => setSelectedId(null)}>
